...
const giftExchangeEventTranslations = {
  ...
  description: {
    tw: '使用轉盤隨機選擇交換禮物對象，增添活動驚喜與樂趣，提供公平透明的抽籤體驗。',
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  const title = giftExchangeEventTranslations.title[locale as keyof typeof giftExchangeEventTranslations.title] || giftExchangeEventTranslations.title.tw;
  const description = giftExchangeEventTranslations.description[locale as keyof typeof giftExchangeEventTranslations.description] || giftExchangeEventTranslations.description.tw;
