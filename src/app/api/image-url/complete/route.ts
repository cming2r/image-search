...
export async function POST(request: NextRequest) {
  try {
    ...
    // 步驟 3：確認上傳完成
    console.log('Step 3: Confirming image upload completion...');
    const completeResponse = await fetch('https://vvrl.cc/api/external/image/upload-complete', {
      method: 'POST',
      headers: {
        'X-API-Key': API_KEY,
    ...
    if (!completeResponse.ok) {
      ...
      console.error('Image upload confirmation failed:', errorText);
      return NextResponse.json(
        { 
          success: false, 
    ...
    if (result.success) {
      console.log('Image upload confirmation completed successfully!');
      return NextResponse.json({
        success: true,
        data: result.data
      });
    ...
    console.error('Image upload confirmation error:', error);
    return NextResponse.json(
      { 
        success: false, 
